...
class ElementExtractor:
    ...
    async def _handle_tool_call(self, tool_call,image_path,step,max_rounds) -> str:
        ...
        if result_list[0]==False:
            ...
            if result_list[0]==False: #重试一次
                return [False, "`image_zoom_and_ocr_tool` failed."]
    ...
    async def run_batch(
        ...
        """
        Modified run_batch:
        Expects `inputs` to be a list of dictionaries with standard keys:
        ...
        async def worker(item):
            async with sem:
                # Direct mapping, no dataset-specific parsing here
        ...
        # Filter out exceeded rounds / failed samples
...
#     for i in range(8):
#         results = asyncio.run(agent.run_batch(inputs))
#         #print(results)
#         output_path = f"/mnt/shared-storage-user/madongsheng/Agent/train_codevision/{i}.json"
